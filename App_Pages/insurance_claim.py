...
def insurance_claim_page():
    ...
    with c1:
        ...
        if ins_num.isdigit() and len(ins_num) == 5:
            ...
            if st.session_state.last_loaded_insurance != insurance_numyear:
                ...
                row = fetch_claim(insurance_numyear)
                ...
                if not row:
                    ...
                    awb = str(row["awbNum"] or "")
                    st.session_state.data = {
                        "awb1": awb[:3],
                        "awb2": awb[3:],
                        "awb_date": row["awbDate"],
    ...
    # ================= AWB INFO =================
    st.markdown("**AWB Info**")

    c1, c2, c3, c4, c5, c6 = st.columns([1, 1, 1.5, 3, 1.5, 1.5])
    with c1:
        awb1 = st.text_input("AWB (Prefix)", value=d.get("awb1", ""), max_chars=3)
    with c2:
        awb2 = st.text_input("AWB (Serial)", value=d.get("awb2", ""), max_chars=8)
    with c3:
        awb_date = st.date_input("AWB Date", value=d.get("awb_date") or None)
    with c4:
    ...
    if add_clicked:
        ...
        ok, msg = insert_claim(st.session_state.data)

        if ok:
            insert_log(action="Inserted New Record",insurance_num=ins_num)
            st.success(msg)
            st.session_state.last_loaded_insurance = None
            st.rerun()
        else:
    ...
    if update_clicked:
        ...
        ok, msg = update_claim_by_insurance(ins_num, st.session_state.data)
        if ok:
            insert_log(action="Updated Record",insurance_num=ins_num)
            st.success(msg)
            st.rerun()
        else:
            st.error(msg)
    ...
    if st.session_state.get("perm_id") == 1:
        ...
        if delete_clicked:
            ok, msg = delete_claim_by_insurance(ins_num)

            if ok:
                insert_log(action="Deleted Record",insurance_num=ins_num)
                st.success(msg)
                st.session_state.data = {}
        ...
        for file in st.session_state.attachments:
            ...
            with col8:
                ...
                if st.button("🗑️", key=f"del_btn_{file['attachID']}", help="Delete this file", type="tertiary", width="stretch"):
                    ...
                    if ok:
                        st.toast(msg) # Smaller notification than st.success
